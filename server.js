...
// Error-proof Root Route
app.get('/', (req, res) => {
  const fileInPublic = path.join(publicPath, 'instructor.html');
  const fileInRoot = path.join(process.cwd(), 'instructor.html');

  res.sendFile(fileInPublic, (err) => {
    if (err) {
      res.sendFile(fileInRoot, (err2) => {
        if (err2) {
          res.status(200).send('<h1>Server Running</h1><p>Upload instructor.html to public/ to view dashboard.</p>');
        }
      });
    }
  });
